...
class TestExtractDocCreatedAt:
    ...
    def test_pptx_created_date(self, tmp_path):
        """PPTX with core_properties.created returns that date."""
        ...
        with patch("pptx.Presentation") as mock_prs_cls:
            mock_prs = MagicMock()
            mock_prs.core_properties.created = dt
            mock_prs_cls.return_value = mock_prs
            result = self._call(pptx_path, ".pptx")
