...
def test_ingest_job_success(ingest_run_config):
    ...
    with (
        ...
        patch("rag_api.pipeline.steps.parse.download_by_key"),
        patch("rag_api.pipeline.steps.parse.SimpleDirectoryReader") as mock_reader,
        patch("rag_api.pipeline.steps.embed.build_embed_model") as mock_embed,
        patch("rag_api.pipeline.steps.embed.build_sparse_model", return_value=None),
