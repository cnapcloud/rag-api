...
def test_ingest_job_success(ingest_run_config):
    ...
    with (
        ...
        patch("rag_api.pipeline.step.parse.download_by_key"),
        patch("rag_api.pipeline.step.parse.SimpleDirectoryReader") as mock_reader,
        patch("rag_api.pipeline.step.embed.build_embed_model") as mock_embed,
        patch("rag_api.pipeline.step.embed.build_sparse_model", return_value=None),
...
def test_ingest_job_validate_passes(ingest_run_config):
    ...
    with (
        patch("rag_api.infra.postgres.get_doc_by_id", return_value=doc),
        patch("rag_api.infra.postgres.update_doc_fields"),
        patch("rag_api.pipeline.step.parse.parse", return_value=[]),
        ...
        patch("rag_api.pipeline.step.chunk.chunk", return_value=[MagicMock()]),
        patch("rag_api.pipeline.step.embed.embed", return_value=[]),
        patch("rag_api.pipeline.step.upsert.upsert") as mock_upsert,
        patch("rag_api.pipeline.step.meta.set_indexed"),
    ):
        from rag_api.pipeline.step.upsert import UpsertResult
