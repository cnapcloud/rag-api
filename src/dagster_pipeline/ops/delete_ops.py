"""Dagster @op — 문서 삭제 파이프라인."""

from dagster import Config, HookContext, OpExecutionContext, failure_hook, op


@failure_hook
def delete_failure_hook(context: HookContext) -> None:
    """Mark document as delete_failed in Redis when any delete op fails."""
    try:
        op_config = context.run_config.get("ops", {}).get("delete_chunks_op", {}).get("config", {})
        kb_id: str = op_config.get("kb_id", "")
        object_key: str = op_config.get("object_key", "")
        if not kb_id or not object_key:
            return
        from pipeline.ops.meta import set_failed
        set_failed(kb_id, object_key, f"delete_job op failed: {context.op_def.name}", run_id=context.run_id)
    except Exception as e:
        context.log.error("delete_failure_hook error: %s", e)


class DeleteConfig(Config):
    kb_id: str
    object_key: str


@op
def delete_chunks_op(context: OpExecutionContext, config: DeleteConfig):
    """Qdrant에서 doc_key 필터로 청크 전체 삭제."""
    from infra import qdrant as qdrant_infra

    qdrant_infra.delete_chunks_by_doc(config.kb_id, config.object_key)
    context.log.info("Qdrant chunks deleted: %s/%s", config.kb_id, config.object_key)
    return {"kb_id": config.kb_id, "object_key": config.object_key}


@op
def delete_meta_op(context: OpExecutionContext, delete_result: dict):
    """Redis 문서 메타데이터 삭제."""
    from infra import redis as redis_infra

    redis_infra.delete_doc_meta(delete_result["kb_id"], delete_result["object_key"])
    context.log.info("Redis meta deleted: %s/%s", delete_result["kb_id"], delete_result["object_key"])