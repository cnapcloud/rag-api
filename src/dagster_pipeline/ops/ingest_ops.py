...
def ingest_failure_hook(context: HookContext) -> None:
    ...
    try:
        ...
        set_failed(kb_id, object_key, f"ingest_job op failed: {context.op_def.name}", run_id=context.run_id)
...
def validate_op(context: OpExecutionContext, config: IngestConfig):
    ...
    if should_process:
        context.log.info("Validation passed: %s/%s", config.kb_id, config.object_key)
        yield Output(
            {
                "kb_id": config.kb_id,
                "object_key": config.object_key,
        ...
        context.log.info("ETag unchanged, skipping: %s/%s", config.kb_id, config.object_key)
...
def parse_op(context: OpExecutionContext, valid_config: dict):
    ...
    documents = parse(kb_id=valid_config["kb_id"], object_key=valid_config["object_key"])
...
def upsert_op(context: OpExecutionContext, valid_config: dict, embedded_nodes):
    ...
    result = upsert(
        ...
        object_key=valid_config["object_key"],
