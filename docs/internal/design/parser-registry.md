...
**서로 무관한 플러그인이 같은 확장자를 다툴 때**: 위 예시는 플러그인 하나가 리더 교체와
post-processor 추가를 함께 하는 경우다. 반대로 **서로의 존재를 모르는 두 개의 독립 플러그인이
같은 확장자에 대해 각자 `register_parser()`를 호출**하면, 리더는 `_parsers`가 dict이므로
마지막에 로드된 플러그인 것만 남고 앞 플러그인이 등록한 리더는 조용히 사라진다 — 앞 플러그인이
그 확장자에 대해 뭔가 등록을 시도했다는 사실 자체가 로그 외에는 드러나지 않는다. 반면
post-processor는 `_post_processors`가 list이므로 두 플러그인이 각각 등록한 함수가 **모두**
실행된다. 즉 같은 확장자를 두고 두 플러그인이 부분적으로만 겹치면(예: 둘 다 `.pdf` 리더를
등록하지만 각자 다른 post-processor도 추가) 리더는 배타적으로 하나만, post-processor는
가산적으로 전부 살아남는 비대칭이 발생한다. 여러 플러그인을 동시에 켤 때는 이 비대칭을 감안해
`parser_plugins` 순서를 의도적으로 정해야 한다.
