...
- **정밀화 레이어 (3·4단계):** 탐지 레이어에서 확정된 후보에 대해 벡터스토어로 확인하고 LLM에 판단까지 요청하는 점점 정밀하고 비싼 방법. **(4단계는 pending — 현재는 3단계까지만 운영, 3.3.3 참고)**
...
두 방식 모두 `pipeline/steps/dedup/` 의 동일한 순수 함수를 공유한다.
...
> **3단계 도입 후 (4단계 pending 기간):** 1·2단계의 `similar` 신호는 위 표처럼 즉시 최종 verdict로 쓰이지 않고, 3단계(청크 비교) 라우팅 트리거로만 쓰인다. 3단계 진입 후에는 3.3.3의 집계 점수 임계값 판정이 body(동일/유사/무관)를 확정해 위 Verdict 결합 규칙에 대입된다. 4단계(비율 기반 세분화 + LLM 확인, "관련" 분류 포함)가 활성화되면 이 임시 판정 대신 3.4 설계가 그 자리를 대체한다.
...
6. **청크 수 비율(chunk_ratio)로 스케일링**: `chunk_ratio = min(A청크수, C청크수) / max(A청크수, C청크수)`, `score(A,C) = raw_score(A,C) * chunk_ratio`. `chunk_ratio`가 이미 `body_similar_threshold` 미만이면 raw_score가 1.0이어도 스케일링 후 threshold를 못 넘으므로, 이 경우 3~5단계(임베딩/벡터스토어 조회)를 생략하고 `score=0.0`으로 즉시 확정한다 (구현: `pipeline/steps/dedup/chunk_compare.py::compare_chunks`)
...
### 3.3.3 청크 비교 결과 기반 임계값 판정 (body 확정)

**목적:** 4단계(3.4, 비율 기반 세분화 + LLM 확인)가 pending인 현재, 3단계의 집계 점수만으로 body(동일/유사/무관)를 확정해 5단계로 넘긴다.

**적용 라우팅 (4단계 pending 기간 — 근본 구조는 2절 다이어그램과 동일, 여기서는 판정 결과 관점으로 명시)**
1. 1단계 SimHash가 `similar`(근접 후보) → 3단계 청크 비교로 라우팅
2. 1단계 SimHash가 후보 없음(`none`) → 2단계 MinHash로 진행
3. 2단계 MinHash가 `similar`(후보 통과) → 3단계 청크 비교로 라우팅
...
| 집계 점수 구간 | body 판정 |
|---|---|
| ≥ body_identical_threshold (0.95) | `identical` |
| body_similar_threshold (0.75) ~ body_identical_threshold 미만 | `similar` |
| < body_similar_threshold (0.75) | `none` |

- `body_identical_threshold`(0.95)/`body_similar_threshold`(0.75)는 3.4 청크쌍 밴드표와 동일한 값을 재사용 — 새 수치 도입 대신 기존 경계와 일관성 유지
- 산출된 body는 2.1절 Verdict 결합 규칙(body × title)에 그대로 대입되어 최종 verdict 산출 → 5단계 전달

**출력**
- (A doc_id, C doc_id, body 판정) → 2.1절 Verdict 결합 규칙 경유 → 5단계

**완료 기준:** 3단계까지만 거친 A-C 쌍이 identical/similar/none 중 하나로 확정되고, title 판정과 결합해 verdict가 산출됨을 확인

**결정 사항**
- 4단계는 pending — 도입 전까지는 본 절의 임계값 판정이 body 확정을 전담한다. 4단계가 활성화되면 3.3.2의 매칭 목록이 3.4로 전달되도록 전환하고, 본 절(3.3.3)은 그 전까지의 임시 대체 로직으로 유지한다.
- 4단계 pending 기간에는 "관련" verdict가 발생하지 않는다 — 관련은 4단계의 비율 기반 판정에서만 산출되는 카테고리이기 때문.

**오픈 이슈**
- body_identical_threshold(0.95)/body_similar_threshold(0.75)는 3.4 밴드값을 재사용한 초기 default — 별도 데이터셋 검증 안 됨
- 4단계 도입 시점/필요성 재검토 — "관련" 관계 추적이나 LLM 확인이 필요해지면 3.4 설계를 그대로 활성화

---

### 3.4 [4단계] 관계 유형(동일/유사/관련/무관) 판정 (pending — 현재 도입 계획 없음, 3.3.3 참고)
...
- **(4단계 pending 기간)** 3단계 직행(3.3.3): (A doc_id, C doc_id, 판정 ∈ {동일, 유사, 무관}, 근거) — "관련"은 4단계 도입 전까지 발생하지 않음
