...
```
 ...
 unregister(evt, cb) ──►│                                                      │
        ...
        │   docs.upload_docs_batch : 현재+남은 항목 결과에 error 표시 후 break │
...
| 포획자 | `connectors._run_sync`, `docs.upload_docs_batch` | `HookAbort`를 잡아 경로별로 정상 처리 |
...
| `api/routers/docs.py` | `upload_docs_batch` | 루프 내 `existing is None`; `HookAbort` 포획 |
