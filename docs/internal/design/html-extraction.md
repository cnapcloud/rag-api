...
`parse_op`의 `HTMLCleanReader`(`src/rag_api/pipeline/steps/parser/html.py`)는 현재 태그 이름 기반
...
- 청킹 품질: 현재 `chunk_op`(`pipeline/steps/chunk.py`)은 HTML 문서도 다른 문서 타입과 동일하게
...
- 근거: `defs/`(Dagster op 래퍼), `pipeline/steps/runner.py`, `_get_file_extractor()` 어느 쪽도
...
| 컴포넌트 | 영향 |
|---|---|
| `pipeline/steps/parser/html.py::HTMLCleanReader` | 내부 구현 교체 (3.4) — US-42에서 `parse.py`에서
  `parser/html.py`로 이동, 로직은 무변경 |
| `pipeline/steps/parser/registry.py::_register_defaults()` | 변경 없음 (`.html`/`.htm` → `HTMLCleanReader` 매핑 유지) |
