...
| Upload batch | `POST /api/kb/{kb_id}/docs/upload/batch` — collect-and-continue. 전부 성공 시 202 `{ results: [{ doc_id, source, etag, status_url }] }`. 실패 항목이 있으면 403(훅 중단) 또는 422, 본문 `{ results: [...], detail: "<한 줄 사유>" }` — 성공 항목 `{ doc_id, ... }`, 실패 항목 `{ title, error, status: "error" }`, `results`는 제출한 파일과 1:1. 프론트: `${성공수} of ${results.length} file(s) uploaded` (중립색) + `detail` 을 그 아래 빨간 한 줄(넘치면 truncate + 툴팁)로. |
