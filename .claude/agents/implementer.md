...
4. **이슈 발생 시 정지** — "Task 현황" 상태를 `blocked`로 갱신하고, "진행 기록"에
   `blocked` 블록(AC, 유형, 이유)을 append한다 — 반환값에만 담으면 세션 종료 시
   사라지므로 파일에도 반드시 남긴다. 반환값에도 요약해 담는다. 
5. **전체 완료** — (모든 task가 `blocked` 없이 `done`일 때만 도달) `uv run ruff check .`로
   전체 레포 스타일을 마지막으로 한 번 확인한 뒤 종료한다.
6. **반환** — `spec.md`/`index.md`는 건드리지 않았음을 확인하고, 완료 목록·
