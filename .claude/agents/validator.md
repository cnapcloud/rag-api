...
2. **architecture 재검증 먼저 수행** — design.md "영향 레이어/파일" 표를 `architecture`
   스킬의 레이어 표·의존성 방향과 대조한다. 표에 파일이 여러 개면 하나씩 순차로 열지
   말고, 한 메시지에서 Read/Grep을 여러 번 동시에 호출해 묶는다:
   - 표에 적힌 파일이 실제로 그 레이어에 속하는지.
...
3. **AC별 실제 검증** — 각 AC는 "테스트 1개 또는 수동 확인 1개와 1:1 대응"하도록
   ...
   - 테스트로 검증 가능하면 실제로 실행한다: `uv run pytest -q <경로>` (전체 회귀는
     `make test`). 여러 AC가 같은 테스트 경로에 매핑돼 있으면 그 경로는 한 번만
     실행하고 결과를 해당 AC들에 나눠 반영한다 — 같은 테스트를 AC 수만큼 재실행하지
     않는다. 통과해야 PASS.
