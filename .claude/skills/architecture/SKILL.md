---
name: architecture
description: backlog/plan 작성 시 레이어 구조·의존성 방향과 모순되지 않는지 확인할 때 사용. analyst·designer 전용, implementer는 사용하지 않음.
---
...
- **analyst**: README.md 개요 + `data-schema.md`만 훑어 요청이 기존 구조/스키마와
  모순되는지만 판단한다. 모순이 있으면 backlog를 쓰기 전에 먼저 보고한다.
- **designer**: 위 레이어 표와 의존성 방향을 기준으로, plan.md에 "이 작업이 어느 레이어의
...
**implementer는 이 스킬을 쓰지 않는다** — designer가 plan.md에 이미 적어둔 레이어/파일
결정을 그대로 따르면 된다. 이 결정이 맞는지는 validator가 완료 기준 검증 시 이 스킬을
기준으로 다시 확인한다(플랜의 레이어 판단 오류를 뒤에서 걸러내는 역할).
