...
9. `index.md`의 "마지막 채번 번호"를 방금 정한 US-NN으로 즉시 갱신한다 (analyst 호출 전에
   먼저 갱신 — 번호가 실제로 쓰였다는 사실을 먼저 기록. 11번에서 analyst가 "중복"으로
   판단해 중단하더라도 이 번호는 재사용하지 않고 버린다 — 번호 낭비보다 재사용 충돌이 더
   위험하다). 같은 시점에 "진행 중" 표에 `| US-NN | <요청 설명 원문> | todo |` 한 줄도
   이 세션이 직접 추가한다 — analyst 호출 전에 먼저 기록해, analyst 호출이 끝까지 못 가고
   끊기더라도(세션 중단 등) 그 사실이 Status `todo`로 남게 한다. 제목 칸은 analyst
   완료 후 spec.md 제목으로 갱신한다(12번).
...
11. analyst의 반환 결과로 분기한다:
    - **중복 판단** ("기존 US-NN에 통합 제안"으로 종료) — spec.md를 채우지 않은 채
      ...
      두지 않는다. 9번에서 추가한 "진행 중" 표의 행도 같이 지운다(폴더/번호와 마찬가지로
      재사용하지 않으므로 표에 `todo`로 남겨두지 않는다). 승인을 묻지 않는다. 사용자에게
      어느 기존 US로 통합할지 제안만 전달한다.
    - **확인 필요 — 진행 보류** (요구사항이 모호함/서로 다르게 해석 가능, 또는 코드·
      아키텍처와 모순되는 지점 발견) — spec.md를 채우지 않은 채 끝난다. 중복 판단과
      달리 브랜치/폴더/번호는 그대로 둔다(Status도 `todo` 유지) — 요청 자체는 유효하고
      확인/해소만 필요한 것이므로. analyst가 반환한 문제 지점을 사용자에게 그대로
      전달하고, 답을 받으면 그 답을 더해 analyst를 다시 호출한다(10번으로).
    - **spec.md 작성 완료** — 12번으로 진행.
12. `.claude/specs/index.md`의 "진행 중" 표에서 9번에 추가한 행을 찾아 Title을 spec.md
    제목으로, Status를 `todo`에서 `specified`로 갱신한다 — 이 세션이 직접 한다(analyst가
    아니라 command가 한다 — 번호 채번과 같은 이유로 판단이 필요 없는 기계적 갱신).
