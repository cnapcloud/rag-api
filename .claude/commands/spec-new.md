...
9. `index.md`의 "마지막 채번 번호"를 방금 정한 US-NN으로 즉시 갱신한다 (analyst 호출 전에
   ...
   위험하다).
10. `Agent` 툴로 `analyst` 서브에이전트를 호출한다. 전달할 것:
    - spec 폴더 경로 (`.claude/specs/US-NN-<slug>/`)
    - 타입 프리픽스를 제외한 요청 설명 원문
11. analyst의 반환 결과로 분기한다:
    ...
    - **spec.md 작성 완료** — 12번으로 진행.
12. `.claude/specs/index.md`의 "진행 중" 표에 `| US-NN | <spec.md 제목> | todo |` 한 줄을
    이 세션이 직접 추가한다 (analyst가 아니라 command가 한다 — 번호 채번과 같은 이유로
    판단이 필요 없는 기계적 갱신).
13. analyst가 채운 spec.md 내용을 사용자에게 보여주고 승인 여부를 묻는다.
14. 사용자가 승인하면, **이 세션이 직접** spec.md의 `## 승인` 체크박스를 `[x]`로 갱신한다.
    analyst/designer는 이 체크박스를 스스로 바꾸지 않는다 — 셀프 승인 금지.
15. 승인 전까지는 `/spec-design`으로 넘어가지 않는다. 사용자가 수정을 요청하면 analyst를
