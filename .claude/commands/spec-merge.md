...
10. **결과 기록** — 멈추거나 끝나는 모든 경로(4-1 재실행 가드, 5번 충돌, 6번 테스트
    실패, 7번 커밋 실패, 9번까지 전체 성공)에서 `templates/merge.md` 템플릿을 그대로
    따라 spec 폴더에 `merge.md`를 쓴다(이미 있으면 덮어쓴다 — 재실행 시 최신 시도
    결과만 남긴다). 그 내용을 사용자에게 보여준다. `origin`에 push할지는 사용자 지시에
    따른다 — 이 커맨드가 자동으로 push하지 않는다.
