---
description: 승인된 backlog.md 기반으로 plan.md(+필요시 task.md) 작성 — designer 서브에이전트 호출
...
2. **브랜치 확인** — 그 spec의 backlog.md 상단 **상태**가 `done`이 아니면, 현재 git
   ...
   브랜치에 파일이 쓰인다.
3. 그 폴더의 `backlog.md`를 읽는다. `## 승인` 체크박스가 `[x]`가 아니면 여기서 멈추고
   사용자에게 먼저 backlog 승인부터 받으라고 안내한다 — plan.md를 생성하지 않는다.
4. `plan.md`가 이미 있으면(재실행 케이스) 내용을 사용자에게 보여주고 처음부터 다시
   만들지, 이어서 손볼지 확인한다. 없으면 `.claude/templates/plan.md`를 그 폴더 안에
   `plan.md`로 복사한다 (아직 템플릿 그대로 — designer가 채운다).
5. `Agent` 툴로 `designer` 서브에이전트를 호출한다. 전달할 것:
   - spec 폴더 경로 (`.claude/specs/US-NN-<slug>/`)
   - backlog.md 경로 (승인 완료 상태임을 이미 확인했다고 함께 전달)
6. designer의 반환 결과로 분기한다:
   - **모순/블로킹 이슈 발견** (예: backlog가 현재 코드/아키텍처와 충돌) — plan.md를
     채우지 않은 채 끝난다. 이슈를 그대로 사용자에게 전달하고 backlog.md 수정이 필요한지
     확인한다. 상태 갱신(7번)으로 넘어가지 않는다.
   - **plan.md 작성 완료** (task.md 포함 여부는 designer 판단) — 7번으로 진행.
7. backlog.md 상단 **상태**를 `in-progress`로 갱신하고, `.claude/specs/index.md`의
   "진행 중" 표에서 해당 US 행의 Status도 `in-progress`로 동기화한다 — 이 세션이 직접
   한다 (designer는 이 두 파일을 갱신하지 않는다).
8. designer가 작성한 plan.md(및 task.md가 있으면 그 내용도)를 사용자에게 보여준다.
   plan.md/task.md에는 backlog.md 같은 별도 승인 체크박스가 없으므로 확인은 필수 게이트가
   아니지만, 사용자가 수정을 요청하면 designer를 다시 호출하거나 직접 고쳐 반영한다.
