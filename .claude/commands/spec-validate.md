...
3. **blocked 게이트** — `implementation.md`의 "Task 현황" 표에 `blocked` 상태인
...
4. `Agent` 툴로 다음 경로를 가지고 `validator` 서브에이전트를 호출한다.
   - spec 폴더 경로
   - `spec.md`/`design.md`/`task.md`/`implementation.md` 경로
5. validator는 실패를 `regression-triage` 스킬의 세 목록으로 구분해 반환한다(각각
   없으면 "(없음)"). 그 스킬의 "기록 위치"/"index.md 갱신"/"사용자 안내"대로 처리한다
...
6. **전체 통과 시 `validated` 전환** — `traceability` 스킬의 문서 정합성 체크리스트를 따른다:
   ...
   - `.claude/specs/index.md`의 "진행 중" 표에서 해당 행 Status를 `validated`로 갱신한다
     ...
     몫이다.
7. validator가 작성한 `validation.md` 내용을 사용자에게 보여주고, `/spec-pr`로
