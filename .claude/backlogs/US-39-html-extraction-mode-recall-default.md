# US-39: HTML 추출 모드(precision/recall/balanced) 설정화 + 기본값 recall 전환
...
`favor_precision`/`favor_recall`은 실제로는 `precision`/`recall`/`balanced` 3단계 중 하나를
고르는 것과 같고(`recall`이 우선), 이 tri-state를 그대로 설정으로 노출해 운영자가 선택할 수
있게 하고 기본값을 `recall`로 전환한다.

## 모드 정의 (요건)

세 모드는 "본문인지 애매한 블록을 얼마나 적극적으로 포함시키는가"라는 하나의 축 위에 있다
(`trafilatura==2.1.0` 소스 확인, 상세 표는 [html-extraction.md §3.2](../../docs/internal/design/html-extraction.md#32-추출-모드--precision--recall--balanced-us-39-2026-07-13-갱신) 참고):

| 모드 | 판단 기준 | 대표 트레이드오프 |
|---|---|---|
| `precision` | 애매하면 제외 (PRECISION_DISCARD_XPATH 등 추가 삭제 규칙 적용) | 본문 일부가 boilerplate로 오판되어 손실될 수 있음 — namu.wiki 사례에서 90%+ 손실 실측 |
| `balanced` | 표준 임계치, 모드 전용 강화/완화 로직 미적용 | precision/recall 중간 — 이번 이슈의 직접 원인은 아니었으나 검증되지 않은 중립값 |
| `recall` (기본값) | 애매하면 포함 (`<p>` 전량 소실 시 정리 롤백, teaser 삭제 생략 등) | 라이선스 푸터 등 짧은 boilerplate가 본문에 섞여 들어올 수 있음 — 실측 확인됨 |

## 범위

- `IngestionSettings.html_extraction_mode: Literal["precision","recall","balanced"] = "recall"`
  추가 — 기존 `html_favor_precision: bool` 필드 대체
- `HTMLCleanReader.load_data()`(`pipeline/ops/parse.py`)가 mode 값에 따라
...
- `tests/unit/test_parse_html.py` 갱신: 기존 `html_favor_precision` 단일 필드 테스트를
  3-모드 매핑 테스트로 교체, "no extractable content" fixture를 (recall 모드에서도 항상
  빈 결과가 나오는) 진짜 무텍스트 케이스로 교체
- `docs/internal/design/html-extraction.md` §3.2(모드 선택 근거)/§6(오픈 이슈) 갱신
...
- `connectors/web.py`의 `_has_sufficient_content()`는 현재 trafilatura를 옵션 없이(중립
  ...
  파싱 단계(`recall`)와 게이팅 단계(중립)가 서로 다른 기준을 쓰는 비일관성은 오픈 이슈로 남김

## 완료 기준

- [x] `html_extraction_mode`의 3개 값(`precision`/`recall`/`balanced`)이 각각 올바른
      `favor_precision`/`favor_recall` kwargs 조합으로 trafilatura에 전달됨 (테스트)
- [x] 기본값 `recall`로 변경한 후에도 기존 HTML 파싱 동작(boilerplate 제외, markdown 구조
      보존, no-content 케이스에서 빈 텍스트 반환)이 유지됨
- [x] `settings.yaml` 계열 4개 파일 모두 `html_extraction_mode` 반영
- [x] `tests/unit/test_parse_html.py` 전체 통과 (unit 스위트 400 passed, 1 skipped)
- [x] 세 모드의 실제 동작 차이(판단 기준/트레이드오프)가 "모드 정의" 절과 설계 문서 §3.2에
...
- WebConnector 게이팅(`_has_sufficient_content`)과 `parse.py` 추출 모드 불일치 — 비범위 참고
