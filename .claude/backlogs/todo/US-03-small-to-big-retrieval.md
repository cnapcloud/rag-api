...
| 파일 | 변경 내용 |
|------|-----------|
| `src/pipeline/steps/chunk.py` | `document_aware` 전략 재추가, 전체 노드 반환 (leaf + parent 구분) |
| `src/pipeline/steps/embed.py` | `parent_node_id`가 없는 노드(leaf)만 임베딩 |
| `src/pipeline/steps/upsert.py` | leaf → 벡터 포함 저장, parent → payload만 저장 분기 |
