...
이 프로젝트는 설계 이후의 작업(backlog 작성 → 구현 계획 → 코드 생성 → 검증)을
`analyst`/`designer`/`implementer`/`validator` 서브에이전트가 단계별로 초안을 만들고,
개발자는 `/spec-new`·`/spec-design`·`/spec-implement`·`/spec-validate` 커맨드로 각 단계를
호출·검토하는 방식으로 진행한다 — 기능/버그 수정을 코드부터 시작하지 않는다. 요건(prd) →
설계(design) → 작업 단위(backlog) → 구현 계획(plan/task) → 코드 → 검증 순으로 이어지는 스펙
체계를 따르고, 각 단계는 문서 상단의 고정된 링크로 서로 연결되어 있어야 한다. 목적은 "왜
이렇게 만들었는지"를 나중에(다른 개발자든 AI든) 코드만 보고는 알 수 없을 때, 문서를 몇 단계만
거슬러 올라가면 찾을 수 있게 하는 것이고, 동시에 AI가 만든 산출물을 개발자가 무엇을 기준으로
...
설계까지는 개발자가 담당하고, 그 이후(backlog → plan/task → 코드 → 검증)는 각 단계 전용
서브에이전트가 초안을 만든다. 승인 게이트는 backlog 단계 하나뿐이지만, 나머지 단계도
...
4. **`/spec-design`** — `designer`가 승인된 backlog를 바탕으로 `plan.md`(영향 레이어/파일 +
   세부 기능별 구현 방법)를 쓰고, 세부 기능이 3개 이상이거나 여러 세션에 걸칠 것 같으면
   `task.md`도 만든다(모든 backlog AC가 어느 task에 매핑되는지 커버리지까지 확인). `plan.md`는
   별도 승인 체크박스가 없어 필수 게이트는 아니지만, 검토를 원하면 이 시점에 한다.
5. **`/spec-implement`** — `implementer`가 task(또는 세부 기능) 단위로 순회하며 구현+테스트를
   진행한다. 이슈 없이 통과하면 자동으로 다음 항목으로 넘어가고, plan.md 설계 결함 등 이슈를
   만나면 그 자리에서 멈춰 개발자 판단을 요청한다(상태가 `blocked`로 바뀐다).
6. **`/spec-validate`** — `validator`가 backlog의 모든 완료 기준(AC)을 실제로 테스트/수동
   확인으로 검증하고, `plan.md`의 레이어/파일 판단도 architecture 기준으로 재검증한다. 전체
   통과하면 **개발자 확인 없이 자동으로** backlog **상태**를 `done`으로 바꾸고
...
- 세부 기능(F1, F2, ...)마다 완료 기준이 뭉뚱그린 문장이 아니라, 실제 테스트 케이스/수동
  ...
  있는지 — 이 ID는 이후 `plan.md`/`task.md`/`validator`가 그대로 재사용하므로 한 번 붙이면
  재번호를 매기지 않는다.

plan.md/task.md는 backlog와 같은 spec 폴더 안에 있으므로(구식 방식처럼 별도 번호/슬러그를
맞출 필요가 없다) 이 시점엔 확인할 것이 없다 — `designer`가 세부 기능 3개 미만·단일
세션이면 자동으로 task.md를 생략한다.
